...
function clear(){pause();revision++;busy=false;result=null;index=0;visited.clear();path.clear();$('comparison').replaceChildren();document.querySelectorAll('.metrics strong').forEach(e=>e.textContent='—');$('status').textContent=ready?'Ready. Paint terrain, then run or compare.':'Loading Python in your browser. First launch may take a moment…';controls();}
...
function advance(){if(!result)return;if(index<result.expanded.length)visited.add(key(result.expanded[index++]));$('status').textContent=`Exploration playback: ${index} / ${result.expanded.length} expanded states.`;if(index===result.expanded.length){pause();path=new Set(result.path.map(key));metrics(result);$('status').textContent=result.cost===null?'No route exists. Try erasing a wall.':'Goal reached. Gold marks the minimum-cost path.';}draw();controls();}
